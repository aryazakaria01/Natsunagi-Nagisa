...
def warn(user: User,
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # punch
            ...
            reply = (
                f"Yep! User {mention_html(user.id, user.first_name)} with extraction database [<code>{user.id}</code>] has been Kicked")

        else:  # ban
            chat.kick_member(user.id)
            reply = (
                f"Yep! User {mention_html(user.id, user.first_name)} with extraction database [<code>{user.id}</code>] has been Banned")
